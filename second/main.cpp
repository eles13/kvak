...
{
    MPI_File file;
    if (MPI_File_open(MPI_COMM_WORLD, args, MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &file))
    {
        if (rank == 0)
...
complexd *f(complexd *A, int n, int k, complexd *H)
{
	...
	if (k > size_2n) {
		...
		MPI_Sendrecv(A, m, MPI_DOUBLE_COMPLEX, rank_, 0, buf, m, MPI_DOUBLE_COMPLEX, rank_, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		if (rank < rank_) {
			for (unsigned long long i = 0; i < m; ++i)
